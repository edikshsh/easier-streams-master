import { ArrayJoinTransform, ErrorTransform, objectTransformsHelper, pipeHelper, SimpleAsyncTransform, SimpleTransform, StreamError, TransformsHelper, TypedEventEmitter } from 'easier-streams-file'


(async () => {
    const src = objectTransformsHelper.fromIterable([1, 2, 3, 4, 5, 6, 7, 8, 9]);
    const action = objectTransformsHelper.fromFunction(async (n: number) => n + 1);
    const dest = objectTransformsHelper.async.fromFunction((n: Promise<number>) => n);

    const pipe = pipeHelper.pipe({}, src, action, dest);
    dest.on('data', (data) => {
        console.log(data);
    })
